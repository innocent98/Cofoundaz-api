...
help:
	...
	@echo ""
	@echo "Production stack (docker-compose.prod.yml + .env.production):"
...
# Production stack
# =============================================================================
# All of these target docker-compose.prod.yml and require a .env.production
# (start from .env.production.example). PROD_COMPOSE is defined once so the
# -f/--env-file pair can never drift between targets -- running one of these
# against the DEV compose file by accident is exactly the kind of mistake that
# wipes a database.
PROD_COMPOSE = docker compose -f docker-compose.prod.yml --env-file .env.production

.env.production:
	@echo "ERROR: .env.production not found."
	@echo "       cp .env.production.example .env.production && chmod 600 .env.production"
	@echo "       Then fill in every CHANGE_ME value."
	@exit 1
...
prod-up: .env.production
	...
	@for i in $$(seq 1 40); do \
		if curl -fsS --max-time 5 http://127.0.0.1:$${API_PORT:-8000}/api/v1/health/ready >/dev/null 2>&1; then \
			echo "READY:"; curl -s http://127.0.0.1:$${API_PORT:-8000}/api/v1/health/ready; echo; exit 0; \
		fi; sleep 3; \
	done; \
