...
`lesson_progress`, `certificates`) in migration `0022_learning`.
...
- `alembic/versions/0022_learning.py` — three brand-new tables, no lock on any existing table.
  Autogenerated from `app/db/models/learning.py`; only the revision id, `down_revision`, Create
  Date and docstring were hand-edited. Chains off `0021_notifications`, the `develop` head when this
  branch was rebased (Module 18 Slice 4 and Module 20 Slice 1 merged first), so the chain runs
  `0018_document_shares` → `0020_signatures` → `0021_notifications` → `0022_learning`.
  `alembic heads` shows `0022_learning` as the single head and `alembic check` reports no drift.
...
- `tests/services/learning/test_service.py` — idempotent enrolment, automatic enrolment, repeat
  completion, the course formula (including 0% for a course with no lessons), one certificate +
  event + job, credential codes, newest-first certificates, per-person and per-workspace isolation.
...
- **Learning unit and API tests: 124 passed** — models 7, migration 2, catalog 6, service 13,
...
- Migration: `alembic upgrade head` applied on the development database; `alembic heads` shows the
  single head `0022_learning`; `alembic check` reports "No new upgrade operations detected."
- **Full project checks — clean, on the latest `develop`.** `ruff check app tests`: all checks
  passed. `black --check app tests`: 309 files unchanged. `mypy app`: no issues in 139 source
  files. Full `pytest` with `--cov-fail-under=95`: **1,160 passed**, coverage **97.97%**. Run from
...
- **Roll back** with `alembic downgrade 0021_notifications`, which drops `lesson_progress`,
