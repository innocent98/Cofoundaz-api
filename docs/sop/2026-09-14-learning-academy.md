...
- `alembic/versions/0019_learning.py` — three brand-new tables, no lock on any existing table.
  Autogenerated from `app/db/models/learning.py`; only the revision id, Create Date and docstring
  were hand-edited. Chains off `0018_document_shares`; `alembic heads` shows `0019_learning` as the
  single head and `alembic check` reports no drift.
...
- **Full project checks — clean.** `ruff check app tests`: all checks passed. `black --check app tests`:
  295 files unchanged. `mypy app`: no issues in 133 source files. Full `pytest` with
  `--cov-fail-under=95`: 1,105 passed and coverage 97.26%; the other 10 — Auth tests that use
  Redis — failed only because the test container looked for Redis at `localhost`. Re-run with
  `REDIS_URL=redis://host.docker.internal:6379/0`, all four Auth test files pass (24 passed), so
  all 1,115 tests pass.
...
- **Roll back** with `alembic downgrade 0018_document_shares`, which drops `lesson_progress`,
