...
> **Type:** incident runbook · **Read time:** 3 minutes · **Last verified:** 2026-08-27
...
> **NOT VERIFIED —** none of these procedures has been executed against a real VPS. They are
> read directly off `.github/workflows/cd.yml` and `docker-compose.prod.yml`. The one part
> that *was* verified is what makes §3 trustworthy: all 7 alembic revisions implement real
> downgrade bodies (`op.drop_table` / `op.drop_index`, not `pass`), and the CI `migrations`
> job proves the `downgrade base` → `upgrade head` round-trip on every run.
...
1. Restores the **previous** `API_IMAGE` value in `.env.production`.
2. Runs `compose up -d --no-build api` to bring the old image back.
3. Polls `/api/v1/health/ready` for up to ~60s and reports whether the rollback is healthy.
4. Dumps the last **80 lines** of `api` logs into the Actions log.
5. Exits 1, so the run is red.

The previous image is read from the **running container** (`compose ps -q api` →
`docker inspect --format '{{.Config.Image}}'`), not from `.env.production`. If an earlier
deploy failed after editing the file but before restarting, the file is a lie and the
container is the truth. It resolves the container via compose rather than a hardcoded name,
so renaming a service or the project cannot silently break rollback.
...
| Case | Consequence |
|---|---|
| **First deploy** — no previous image exists | It restores `.env.production.bak` and reports "no previous image recorded — cannot auto-roll back." Fix forward. |
...
If a stray `.env.production.bak` is left behind, the deploy did not reach its clean exit. Read
it before deleting it — it is the pre-deploy state of the file.
...
(`if: github.event.inputs.image_tag == ''`) and goes straight to deploy — no rebuild, no
CI wait.
...
sed -i "s|^API_IMAGE=.*|API_IMAGE=ghcr.io/innocent98/cofoundaz-api:sha-<good>|" .env.production
$COMPOSE up -d --no-build api
curl -fsS http://127.0.0.1:8000/api/v1/health/ready
```

Note `--no-build`. Without it, compose may try to build from a source tree that is not there.

Record what you did somewhere the next deploy's operator will see it — a manual edit to
`.env.production` is invisible to the pipeline until the next deploy overwrites it.
...
| Editing `.env.production` without a `.bak` | The deploy script takes one for a reason. |
...
- [ ] `docker inspect` on the running container, `API_IMAGE` in `.env.production`, and
      `revision` all agree.
- [ ] `alembic current` matches the schema the deployed code expects.
- [ ] Any stray `.env.production.bak` is read, then removed.
...
- **[GITHUB_ACTIONS_SETUP.md](./GITHUB_ACTIONS_SETUP.md)** — secrets and variables.
